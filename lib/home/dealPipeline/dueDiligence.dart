...
class DueDiligence extends StatefulWidget {
  const DueDiligence({super.key});

  @override
  State<DueDiligence> createState() => _DueDiligenceState();
}

class _DueDiligenceState extends State<DueDiligence> {
    ...
    try {
      ...
      if (!incubeProvider.isAdmin && !incubeProvider.isteamLeader) {
        ...
        for (String s in listOfDealIds) {
          _dealList.add(organization.org_deals
              .where((element) =>
                  element.idDeal == s && element.status == "due diligence")
              .first);
          safePrint('length of tempDealList is: ${tempDealList.length}');
          setState(() {
            _dealList = tempDealList;
        ...
        setState(() {
          _dealList = organization.org_deals
              .where((element) => element.status == 'due diligence')
              .toList();
        });
        safePrint('length of _dealList is: ${_dealList.length}');
      }
